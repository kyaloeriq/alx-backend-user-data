...
class BasicAuth(Auth):
    ...
    def current_user(self, request=None):
        ...
        if auth_header is None:
            return None
        user_credentials = self.extract_base64_authorization_header(auth_header)
        if user_credentials is None:
            return None
        user_email, user_password = self.decode_base64_authorization_header(user_credentials)
        user = User.search({'email': user_email})  # Fetch the user by email
        if user is None or not user.is_valid_password(user_password):
            return None  # User doesn't exist or password is invalid
        return user
